...
app
    .controller('monitoringCtrl', function ($scope,$http) {
        ...
        $http.post("/monitoring/GetInfo").then(function(response){
            if(response.data.res=='yes'){ console.log(response.data.cpu1);
                   angular.forEach(response.data.cpu1, function(x){
                       seriesData.push(x);
                   });
            }
        });
