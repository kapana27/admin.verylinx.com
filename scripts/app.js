...
.factory('AuthenticationService', ['$http', '$localStorage', '$window', '$timeout', '$location', '$interval', '$rootScope', function ($http, $localStorage, $window, $timeout, $location, $interval, $rootScope) {
	...
	service.Login = function (username, password, callback) {
		...
		.success(function (response) {
			if (response.token) {
				$localStorage.permissions = response.permissions;
				$rootScope.perm = $localStorage.permissions;
				service.setToken(response.token, callback);
			} else {
				callback(false);
			}
		});
	...
	return service;
}]).factory('user', function ($http, $localStorage, onl) {
	var service = {};
	
	service.auth = function (user, callback) {
		$http.post("/wap/getToken/", user).then(function (rs) {
			if (rs.data.res == "yes") {
				service.setToken(rs.data.token, callback);
				$localStorage.accounts = rs.data.accounts;
				console.log($localStorage.accounts);
			} else {
				onl.alert( rs.data.error);
			}
		});
		return true;
	}
	...
	service.setToken = function (token, callback) {
		var base64Url = token.split('.')[1];
		var base64 = base64Url.replace('-', '+').replace('_', '/');
		$localStorage.user = JSON.parse(atob(base64));
		$localStorage.t = token;
		$http.defaults.headers.common.Authorization = 'Bearer ' + token;
		
		service.data = service.getData();
	...
	service.is_logged = function () {
		if ($localStorage.user != undefined && $localStorage.user.id != undefined && $localStorage.user != "") {
			return true;
		}
		return false;
	}
	...
	service.logout = function () {
		...
		top.location.href="#/app/main"
	}
	
	if ($localStorage.t != undefined) {
		service.setToken($localStorage.t);
	}
...
.config(['$stateProvider', '$urlRouterProvider', function($stateProvider, $urlRouterProvider) {
	...
	.state('app', {
		...
		resolve: {
			authenticate: ['$q', '$state', '$timeout', authenticate],
		}
	})
		//dashboard
		.state('app.dashboard', {
